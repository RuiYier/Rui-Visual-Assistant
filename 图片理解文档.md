...
注意：获取 API Key 等准备工作，请参考 [首次调用API](https://platform.xiaomimimo.com/#/docs/quick-start/first-api-call)。
...
> 注：计算图像的 Token 请参考 [图片 Token 用量说明](https://platform.xiaomimimo.com/#/docs/usage-guide/multimodal-understanding/image-understanding?target=图片-token-用量及缩放规则说明)。模型上下文长度请参考 [定价与限速](https://platform.xiaomimimo.com/#/docs/pricing)。
...
- 计费：总费用根据输入、输入（命中缓存）和输出 Token 数计算；价格请参考 [定价与限速](https://platform.xiaomimimo.com/#/docs/pricing)。
- 可通过 [图片 Token 用量说明](https://platform.xiaomimimo.com/#/docs/usage-guide/multimodal-understanding/image-understanding?target=图片-token-用量及缩放规则说明) 计算图片的 Token 消耗。估算结果仅供参考，实际用量以 API 响应为准。
...
`mimo-v2.5` 和 `mimo-v2-omni` 模型暂不支持图片本地文件上传。支持的上传方式请参考 [图片传入方式](https://platform.xiaomimimo.com/#/docs/usage-guide/multimodal-understanding/image-understanding?target=图片传入方式)。