...
> 注：计算视频的 Token 请参考 [视频 Token 用量说明](https://platform.xiaomimimo.com/#/docs/usage-guide/multimodal-understanding/video-understanding?target=视频-token-用量说明)。模型上下文长度请参考 [定价与限速](https://platform.xiaomimimo.com/#/docs/pricing)。
...
- 计费：总费用根据输入、输入（命中缓存）和输出 Token 数计算；价格请参考 [定价与限速](https://platform.xiaomimimo.com/#/docs/pricing)。
  - 可通过 [视频 Token 用量说明](https://platform.xiaomimimo.com/#/docs/usage-guide/multimodal-understanding/video-understanding?target=视频-token-用量说明) 计算视频的 Token 消耗。估算结果仅供参考，实际用量以 API 响应为准。
...
`mimo-v2.5` 和 `mimo-v2-omni` 模型暂不支持视频本地文件上传。支持的上传方式请参考 [视频传入方式](https://platform.xiaomimimo.com/#/docs/usage-guide/multimodal-understanding/video-understanding?target=视频传入方式)。